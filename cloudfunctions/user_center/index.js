...
exports.main = async (event, context) => {
    ...
    if (gardenRes.data.length > 0) {
      ...
      if (myGarden.rose_balance && myGarden.rose_balance > 0) {
        ...
        await db
          .collection("users")
          .doc(me._id)
          .update({ data: { rose_balance: _.inc(oldBalance) } });
        await db
          .collection("gardens")
          .doc(myGarden._id)
    ...
    if (oldLogRes.data.length > 0) {
      ...
      await db.collection("logs").add({
        data: {
          ...
          type: "daily_check_in",
          content: "完成了今日打卡",
          imageFileID: imageFileID, // 确保这个字段被写入
          originalDate: todayStr,
          createdAt: db.serverDate(),
          engine: "tencent",
          style: "success",
  ...
  if (action === "redeem_coupon") {
    ...
    if (balance < cost)
      return { status: 400, msg: `玫瑰不足，还差 ${cost - balance} 朵哦~` };
  ...
  if (action === "make_decision") {
    ...
    const updateData = {
      last_decision: { category, result, time: db.serverDate() },
    };
    await db
      .collection("users")
      .where({ _openid: myOpenID })
